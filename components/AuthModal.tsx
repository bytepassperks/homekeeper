  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/${edgeFunctionName}/make-server-7627b83a/signup`,
        {
          method: 'POST',
          headers: {
      ...
      if (err.message?.includes('CORS') || err.message?.includes('Failed to fetch') || err.name === 'TypeError') {
        setError('Server connection error. The backend may not be deployed. Contact support@homemaker.co.site');
      } else {
