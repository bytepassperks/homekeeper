...
export default function App() {
  ...
  const loadUserData = async (token: string, uid: string) => {
    try {
      ...
      const itemsRes = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a/items`,
        {
          headers: {
      ...
      const prefsRes = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a/preferences`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          }
      ...
      const notifRes = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a/notifications`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          }
      ...
      const logsRes = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a/webhook-logs`,
        {
          headers: {
  ...
  const handleDeleteItem = async (itemId: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a/items/${itemId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
  ...
  const loadMaintenanceHistory = async (itemId: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a/maintenance/${itemId}`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
          }
  ...
  const handleLogMaintenance = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a/maintenance`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
  ...
  const handleMarkForReplacement = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a/items/${selectedItem.id}`,
        {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
  ...
  const handleFindReplacement = async (item: Item) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a/api/webhook/find-replacement`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
  ...
  const handleTestEndpoint = async (path: string) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-7627b83a${path}`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
  ...
  return (
    <>
      ...
      {currentPage === 'faq' && (
        <FAQPage onBack={() => setCurrentPage('landing')} />
      )}
